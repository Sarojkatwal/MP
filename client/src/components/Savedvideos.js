...
export default class Savedvideos extends Component {
    ...
    componentDidMount() {
        setTimeout(() => this.getAllVideos(), 2000)
    }

    getAllVideos = async () => {

        try {
            const response = await fetch('http://localhost:4000/svideos');
        ...
        return (
            <div className="container ">
                {!this.state.loading1 ?
                    ...
                    <div className='spinnerCenter'><div className="spinner-border colorwhite" /></div>
                }
            </div>
        )
    }
